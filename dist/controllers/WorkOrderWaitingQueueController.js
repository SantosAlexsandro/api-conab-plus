...
class WorkOrderWaitingQueueController {
  async index(req, res) {
    try {
      ...
      let workOrders;
      if (status) {
        workOrders = await _WorkOrderWaitingQueueService2.default.findByStatus(status);
      } else {
        // Se não for fornecido status, buscar todos os registros
        workOrders = await _WorkOrderWaitingQueueService2.default.findAll();
      }
  ...
  */
  ...
  async pauseTechnicianAssignment(req, res) {
    ...
    try {
      ...
      await _WorkOrderWaitingQueueService2.default.setEditingFlag(orderNumber, true, userId);
  ...
  async resumeTechnicianAssignment(req, res) {
    ...
    try {
      ...
      await _WorkOrderWaitingQueueService2.default.setEditingFlag(orderNumber, false, userId);
